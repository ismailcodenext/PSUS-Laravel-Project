...
<script>
    ...
    async function getList() {
        try {
            ...
            let res = await axios.get("/api/mission-vision-list", HeaderToken()); // Fetch brand list
            hideLoader(); // Hide loader after response

            let tableList = $("#tableList");
            tableList.empty(); // Clear previous table content

            res.data['MissionVisionData'].forEach(function(item, index) {
                let row = `
                        <tr data-row="${index + 1}">
                            <td>${index + 1}</td>
 <td>
                         ...
                         <td><img style="width: 100px; height: auto" alt="" src="${item['img_url']}"></td>

                            <td>${item['misson_title']}</td>
                            <td>${item['misson_desc']}</td>
                            <td>${item['visions_title']}</td>
                            <td>${item['visions_desc']}</td>
                            <td>${item['status']}</td>
