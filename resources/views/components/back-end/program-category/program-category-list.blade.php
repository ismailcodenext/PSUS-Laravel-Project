...
<script>
    ...
    async function getList() {
        try {
            ...
            let res = await axios.get("/api/program-category-list", HeaderToken()); // Fetch brand list
            hideLoader(); // Hide loader after response

            let tableList = $("#tableList");
            tableList.empty(); // Clear previous table content

            res.data['ProgramCategoryData'].forEach(function(item, index) {
                let row = `
                        <tr data-row="${index + 1}">
                            <td>${index + 1}</td>
 <td>
